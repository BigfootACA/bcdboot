...
int process_change(char*part){
	...
	uuid_t ku,pku;
	guid_t kg,pkg;
	char*kname=NULL,*pkname=NULL,*kpath=NULL,*pkpath=NULL;
	dump_info();
	putchar('\n');
	puts("Resolve block...");
	if(!(kname=get_kname(part))){
		perror("failed to get block kernel name");
		goto fail;
	}
	if(!(pkname=get_pkname(kname))){
		perror("failed to get parent block kernel name");
		goto fail;
	}
	if(!(kpath=find_block(kname))){
		perror("failed to get block kernel path");
		goto fail;
	}
	if(!(pkpath=find_block(pkname))){
		perror("failed to get parent block kernel path");
		goto fail;
	}
	puts("Read UUID...");
	if(!get_uuid(ku,"PARTUUID",kpath)){
		perror("failed to get PARTUUID");
		goto fail;
	}
	if(!get_uuid(pku,"PTUUID",pkpath)){
		perror("failed to get PTUUID");
		goto fail;
	}
	if(!uuid2guid(&pkg,pku)||!uuid2guid(&kg,ku)){
		perror("convert guid failed");
		goto fail;
	}
	puts("Change GUID...");
	...
	if(set_device(def_boot,fget_lib("ApplicationDevice",NULL),&appdev)<0){
		perror("set appdevice");
		goto fail;
	}
	if(hivex_commit(bcd,NULL,0)<0){
		perror("commit changes");
		goto fail;
	}
	puts("Done");
	putchar('\n');
	...
	fail:
	if(kname)free(kname);
	if(pkname)free(pkname);
	if(kpath)free(kpath);
	if(pkpath)free(pkpath);
	return r;
}
